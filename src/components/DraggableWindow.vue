...
<script lang="ts" setup>
  ...
  function elementDrag(event: MouseEvent) {
    ...
    const { width, height, minHeight } = container.value.style;
  ...
  function closeDragElement() {
    ...
    const { left, top, width, height } = container.value.style;
    const chatPosition = {
    ...
    localStorage[`chat_${props.positionBasis}Position`] = JSON.stringify([
      side,
      offset,
      side2,
      offset2,
   ...
   */
  function minimize() {
    const { top, height, minHeight } = container.value.style;
  ...
  function externalClick() {
    // Only receives clicks outside container
  ...
  function resizeDrag(event: MouseEvent) {
    ...
    if (dir.includes('l')) {
      ...
      if (widthValue > minWidthNum)
        container.value.style.left = `${container.value.offsetLeft - resizePositions.movementX}px`;
    }
    if (dir.includes('t')) {
