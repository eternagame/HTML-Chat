...
var myUserName = Application.CURRENT_USER ? Application.CURRENT_USER.name : "Anonymous";
var myUID = Application.CURRENT_USER ? Application.CURRENT_USER.uid : 0;
// Replace invalid IRC nick characters with "-" (or _ if first char), add UID and initial connection number
var myNick = myUserName.replace(/^[^a-zA-Z\x5B-\x60\x7B-\x7D]/, "_").replace(/[^a-zA-Z\x5B-\x60\x7B-\x7D\d-]/g, "-") + "__" + myUID + "^1";
var onlineUsers = [];
var ignored = localStorage.chatIgnored || [];
var autoScroll = true;
// Test server - TODO: Replace
...
function postMessage( raw_msg, isHistory ) {
    ...
    if ( raw_msg.startsWith("!") || ignored.includes(name) ){return;}
...
$( document ).ready(function() {
    ...
    $("#chat-input").prop("maxLength", 460 - myUID.length - myUserName.length);
    ...
    $('#chat-input').keypress(function (e) {
        ...
        if (e.which == 13) {
            ...
            if (myUserName !== "Anonymous" && $('#chat-input').val() !== '') {
                ...
                if (message.startsWith("/")) {
                    ...
                    switch (command) {
                        ...
                        case "ignore":
                            ignored.push(params);
                            localStorage.chatIgnored = ignored;
                        ...
                        case "unignore":
                            if (params == "*") {
                                ...
                                ignored.splice(ignored.indexOf(params), 1);
                ...
                if (isAction) {
                    message = myUID + '_<I><FONT COLOR="#C0DCE7">' + myUserName + "</FONT></I>_" + new Date().toUTCString().replace(/(Mon|Tue|Wed|Thu|Fri|Sat|Sun), ([0-3]\d) (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) (\d{4}) ([0-2]\d(?::[0-6]\d){2}) GMT/, "$1 $3 $2 $5 $4 UTC") + "_<I>" + message + "</I>";
                } else {
                    message = myUID + "_" + myUserName + "_" + new Date().toUTCString().replace(/(Mon|Tue|Wed|Thu|Fri|Sat|Sun), ([0-3]\d) (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) (\d{4}) ([0-2]\d(?::[0-6]\d){2}) GMT/, "$1 $3 $2 $5 $4 UTC") + "_" + message;
                }
    ...
    for (var i=0; i<commands.length; i++) {
        ...
        switch(cmd.command) {
            ...
            case "433":
                ...
                var nickNum = parseInt(myNick.match(/\^(\d+)/)[1]) + 1;
                myNick = myNick.replace(/\^(\d+)/, "^" + nickNum);
                sock.send("NICK " + myNick + "\r\n");
                sock.send("USER " + "anon" + " 0 * :" + myUserName + "\r\n");
                break;
            case "001":
            ...
            case "MODE":
                ...
                if (cmd.params[1] == "+b") {
                    ...
                    if (myNick.match(new RegExp(maskParts[2].replace("*", ".+").replace("^", "\\^")))) {
                    ...
                    if (myNick.match(new RegExp(maskUser.replace("*", ".+").replace("^", "\\^")))) {
                        $("#chat-input").prop('disabled', false);
